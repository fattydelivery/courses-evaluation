# 进入flume安装目录下：
bin/flume-ng agent -c conf/ -n a1 -f /home/hadoop/flume-kafka.conf 

# hive中建表
hive -e 'create external table hbase_courses_evaluation(rowkey string, ip string, time string) stored by "org.apache.hadoop.hive.hbase.HBaseStorageHandler" with serdeproperties ("hbase.columns.mapping"=":key,info:ip,info:time") tblproperties("hbase.table.name"="project:courses-evaluation");
